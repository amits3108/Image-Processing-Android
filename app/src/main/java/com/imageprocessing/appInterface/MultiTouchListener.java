...
import com.imageprocessing.widget.ScaleGestureDetector;
import com.imageprocessing.widget.Vector2D;
...
public class MultiTouchListener implements OnTouchListener {
    ...
    public MultiTouchListener(View deleteView, ImageView photoEditImageView,
                              OnImageProcessingListener onImageProcessingListener) {
    ...
    public boolean onTouch(View view, MotionEvent event) {
        ...
        switch (action & event.getActionMasked()) {
            case MotionEvent.ACTION_DOWN:
                ...
                callImageProcessingListener(view, true);
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (pointerIndexMove != -1) {
                    ...
                    if (!mScaleGestureDetector.isInProgress()) adjustTranslation(view, currX - mPrevX, currY - mPrevY);
                }
            ...
            case MotionEvent.ACTION_UP:
                ...
                callImageProcessingListener(view, false);
                ...
                if (mCurrentCancelX == mPrevRawX || mCurrentCancelY == mPrevRawY) {
                    if (view instanceof TextView) {
                        ...
                        if (onImageProcessingListener != null) {
                            onImageProcessingListener.onEditTextChangeListener(
                                    ((TextView) view).getText().toString(), ((TextView) view).getCurrentTextColor());
                        }
                    }
                }
    ...
    private void callImageProcessingListener(View view, boolean isStart) {
        if (onImageProcessingListener != null) {
            if (view instanceof TextView) {
                if (onMultiTouchListener != null) {
                    setStartStopChangeListener(isStart, EditType.TEXT);
                } else {
                    setStartStopChangeListener(isStart, EditType.EMOJI);
                }
            } else {
                setStartStopChangeListener(isStart, EditType.IMAGE);
            }
        }
    }

    private void setStartStopChangeListener(boolean isStart, EditType text) {
        if (isStart) {
            onImageProcessingListener.onStartViewChangeListener(text);
        } else {
            onImageProcessingListener.onStopViewChangeListener(text);
        }
    }
